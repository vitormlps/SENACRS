...
from sqlalchemy import text, select, insert, Insert, update, delete, Delete
...
class BaseRepo(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def get_index_sequence(self):
        db_data = self.session.execute(
            text("SELECT nextval(:index_seq);"), index_seq=self.__INDEX_SEQUENCE)
    ...
    def __qualify_filters(self, filters, skip, limit):
        ...
        if isinstance(filters, BaseModel):
            ...
            filters = filters.dict(
                exclude_none=True, exclude={'skip', 'limit'})

        else:
            skip, limit = filters.pop("skip"), filters.pop("limit")
            temp_filters = filters.copy()
            [filters.pop(key)
             for key, value in temp_filters.items() if value is None]
    ...
    def update(self, id: str, payload, commit=True) -> ModelType:
        query = update(self.model).where(self.model.id == id).values(
            **payload).returning(self.model)
    ...
    def remove(self, id: str, commit=True) -> Optional[bool]:
    ...
    def truncate(self, commit=True) -> Optional[bool]:
    ...
    def execute_query_all(self, query):
        ...
        try:
            ...
            [self.session.refresh(result[0]) for result in results]
    ...
    def execute_transaction(self, query):
        ...
        try:
            ...
            if isinstance(query, Delete):
                ...
                result = self.session.execute(query).one()[0].__dict__
            else:
                result = self.session.execute(query).one()[0]
                self.session.refresh(result)
